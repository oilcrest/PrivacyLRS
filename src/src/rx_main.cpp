...
// ChaCha12 vs ChaCha20 Hardware Benchmark - Finding #5
// This benchmark measures the real-world performance difference between
// ChaCha12 (current) and ChaCha20 (RFC 8439 standard) on ESP32 hardware
void runChaCha20Benchmark()
{
    Serial.println("\n========================================");
    Serial.println("ChaCha12 vs ChaCha20 Hardware Benchmark");
    Serial.println("Finding #5 - Hardware Performance Test");
    Serial.println("========================================\n");
    ...
    uint8_t plaintext_8[8] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08};
    uint8_t plaintext_14[14] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
                                 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e};
    uint8_t ciphertext[14];

    // Test ChaCha12 with 8-byte packets
    Serial.println("Testing ChaCha12 (8-byte packets)...");
    ChaCha cipher12(12);
    ...
    // Warm-up
    for (int i = 0; i < 10; i++) {
        cipher12.encrypt(ciphertext, plaintext_8, 8);
    }

    uint32_t start12_8 = micros();
    for (uint32_t i = 0; i < ITERATIONS; i++) {
        cipher12.encrypt(ciphertext, plaintext_8, 8);
    }
    uint32_t elapsed12_8 = micros() - start12_8;
    float us_per_op12_8 = (float)elapsed12_8 / (float)ITERATIONS;

    Serial.print("  Time: ");
    Serial.print(elapsed12_8);
    Serial.print(" us for ");
    Serial.print(ITERATIONS);
    Serial.println(" iterations");
    Serial.print("  Average: ");
    Serial.print(us_per_op12_8, 2);
    Serial.println(" us/packet");
    Serial.print("  Throughput: ");
    Serial.print((uint32_t)(1000000.0f / us_per_op12_8));
    Serial.println(" packets/sec\n");

    // Test ChaCha20 with 8-byte packets
    Serial.println("Testing ChaCha20 (8-byte packets)...");
    ChaCha cipher20(20);
    cipher20.setKey(test_key, 32);
    cipher20.setIV(test_nonce, 8);
    cipher20.setCounter(test_counter, 8);

    // Warm-up
    for (int i = 0; i < 10; i++) {
        cipher20.encrypt(ciphertext, plaintext_8, 8);
    }

    uint32_t start20_8 = micros();
    for (uint32_t i = 0; i < ITERATIONS; i++) {
        cipher20.encrypt(ciphertext, plaintext_8, 8);
    }
    uint32_t elapsed20_8 = micros() - start20_8;
    float us_per_op20_8 = (float)elapsed20_8 / (float)ITERATIONS;

    Serial.print("  Time: ");
    Serial.print(elapsed20_8);
    Serial.print(" us for ");
    Serial.print(ITERATIONS);
    Serial.println(" iterations");
    Serial.print("  Average: ");
    Serial.print(us_per_op20_8, 2);
    Serial.println(" us/packet");
    Serial.print("  Throughput: ");
    Serial.print((uint32_t)(1000000.0f / us_per_op20_8));
    Serial.println(" packets/sec\n");

    // Test ChaCha12 with 14-byte packets
    Serial.println("Testing ChaCha12 (14-byte packets)...");
    cipher12.setKey(test_key, 32);
    cipher12.setIV(test_nonce, 8);
    cipher12.setCounter(test_counter, 8);
    ...
    // Warm-up
    for (int i = 0; i < 10; i++) {
        cipher12.encrypt(ciphertext, plaintext_14, 14);
    }

    uint32_t start12_14 = micros();
    for (uint32_t i = 0; i < ITERATIONS; i++) {
        cipher12.encrypt(ciphertext, plaintext_14, 14);
    }
    uint32_t elapsed12_14 = micros() - start12_14;
    float us_per_op12_14 = (float)elapsed12_14 / (float)ITERATIONS;

    Serial.print("  Time: ");
    Serial.print(elapsed12_14);
    Serial.print(" us for ");
    Serial.print(ITERATIONS);
    Serial.println(" iterations");
    Serial.print("  Average: ");
    Serial.print(us_per_op12_14, 2);
    Serial.println(" us/packet");
    Serial.print("  Throughput: ");
    Serial.print((uint32_t)(1000000.0f / us_per_op12_14));
    Serial.println(" packets/sec\n");

    // Test ChaCha20 with 14-byte packets
    Serial.println("Testing ChaCha20 (14-byte packets)...");
    cipher20.setKey(test_key, 32);
    cipher20.setIV(test_nonce, 8);
    cipher20.setCounter(test_counter, 8);
    ...
    // Warm-up
    for (int i = 0; i < 10; i++) {
        cipher20.encrypt(ciphertext, plaintext_14, 14);
    }

    uint32_t start20_14 = micros();
    for (uint32_t i = 0; i < ITERATIONS; i++) {
        cipher20.encrypt(ciphertext, plaintext_14, 14);
    }
    uint32_t elapsed20_14 = micros() - start20_14;
    float us_per_op20_14 = (float)elapsed20_14 / (float)ITERATIONS;

    Serial.print("  Time: ");
    Serial.print(elapsed20_14);
    Serial.print(" us for ");
    Serial.print(ITERATIONS);
    Serial.println(" iterations");
    Serial.print("  Average: ");
    Serial.print(us_per_op20_14, 2);
    Serial.println(" us/packet");
    Serial.print("  Throughput: ");
    Serial.print((uint32_t)(1000000.0f / us_per_op20_14));
    Serial.println(" packets/sec\n");

    // Calculate overhead
    float overhead_8 = ((us_per_op20_8 - us_per_op12_8) / us_per_op12_8) * 100.0f;
    float overhead_14 = ((us_per_op20_14 - us_per_op12_14) / us_per_op12_14) * 100.0f;
    float avg_overhead = (overhead_8 + overhead_14) / 2.0f;

    Serial.println("========================================");
    Serial.println("OVERHEAD ANALYSIS");
    Serial.println("========================================");
    Serial.print("8-byte packets:  +");
    Serial.print(overhead_8, 1);
    Serial.println("%");
    Serial.print("14-byte packets: +");
    Serial.print(overhead_14, 1);
    Serial.println("%");
    Serial.print("Average:         +");
    Serial.print(avg_overhead, 1);
    Serial.println("%\n");

    // CPU usage at different packet rates
    Serial.println("========================================");
    Serial.println("CPU USAGE @ DIFFERENT PACKET RATES");
    Serial.println("========================================");

    float rates[] = {50.0f, 150.0f, 250.0f};
    const char* rate_names[] = {"50Hz", "150Hz", "250Hz"};

    for (int i = 0; i < 3; i++) {
        float interval_us = 1000000.0f / rates[i];
        float cpu12 = (us_per_op12_8 / interval_us) * 100.0f;
        float cpu20 = (us_per_op20_8 / interval_us) * 100.0f;
        float additional = cpu20 - cpu12;

        Serial.print(rate_names[i]);
        Serial.print(" (");
        Serial.print(interval_us / 1000.0f, 2);
        Serial.println("ms interval):");
        Serial.print("  ChaCha12: ");
        Serial.print(cpu12, 3);
        Serial.println("%");
        Serial.print("  ChaCha20: ");
        Serial.print(cpu20, 3);
        Serial.println("%");
        Serial.print("  Additional: +");
        Serial.print(additional, 3);
        Serial.println("%\n");
    }
